
/// EventDispatcher, main class of a lot
/// <reference path="Core/EventDispatcher.ts" />
/// <reference path="Core/Queue.ts" />
/// <reference path="Core/Collection.ts" />
/// <reference path="Core/Hash.ts" />
/// Modules, like fs or path
/// <reference path="Utils/Modules.ts" />

/// All Interfaces (I*.ts)
/// <reference path="Core/IPlugin.ts" />
/// <reference path="LINQ/IProvider.ts" />
/// <reference path="Resource/IFiletype.ts" />
/// <reference path="Resource/ILoadable.ts" />
/// <reference path="Utils/ISettings.ts" />

/// Utils
/// <reference path="Utils/FPSCounter.ts" />
/// <reference path="Utils/KeyMap.ts" />
/// <reference path="Utils/Map.ts" />
/// <reference path="Utils/Noise.ts" />
/// <reference path="Utils/Settings.ts" />
/// <reference path="Utils/Utils.ts" />
/// <reference path="Utils/CanvasUtils.ts" />
/// <reference path="Utils/UpdateChecker.ts" />
/// <reference path="Utils/GameStartParameters.ts" />
/// <reference path="Utils/Tween.ts" />

/// Base Classes
/// <reference path="Entities/Entity.ts" />
/// <reference path="Abilities/Ability.ts" />
/// <reference path="Core/Scene.ts" />

/// All the rest
/// This is auto generated
//grunt-start
/// <reference path="../defs/LINQ.d.ts" />
/// <reference path="../defs/WebGL.d.ts" />
/// <reference path="../defs/gamepad.d.ts" />
/// <reference path="../defs/lib.d.ts" />
/// <reference path="../defs/node.d.ts" />
/// <reference path="../defs/nodeunit.d.ts" />
/// <reference path="../defs/physijs.d.ts" />
/// <reference path="../defs/three.d.ts" />
/// <reference path="../defs/waa.d.ts" />
/// <reference path="HG.ts" />
/// <reference path="Abilities/AnimationAbility.ts" />
/// <reference path="Abilities/MovingAbility.ts" />
/// <reference path="Abilities/ScriptExecuteAbility.ts" />
/// <reference path="Core/BaseGame.ts" />
/// <reference path="Core/PluginHost.ts" />
/// <reference path="Core/Shader.ts" />
/// <reference path="Entities/CameraEntity.ts" />
/// <reference path="Entities/ChasingCameraEntity.ts" />
/// <reference path="Entities/FirstPersonCameraEntity.ts" />
/// <reference path="Entities/MeshEntity.ts" />
/// <reference path="Entities/ParticleEntity.ts" />
/// <reference path="Entities/SkyBoxEntity.ts" />
/// <reference path="Entities/SpriteEntity.ts" />
/// <reference path="Entities/TextEntity.ts" />
/// <reference path="Entities/VideoEntity.ts" />
/// <reference path="Input/GamepadDevice.ts" />
/// <reference path="Input/Handler.ts" />
/// <reference path="Input/IDevice.ts" />
/// <reference path="Input/KeyboardDevice.ts" />
/// <reference path="Input/MouseDeivce.ts" />
/// <reference path="LINQ/ArrayProvider.ts" />
/// <reference path="LINQ/FunctionProvider.ts" />
/// <reference path="LINQ/NumberProvider.ts" />
/// <reference path="LINQ/ObjectProvider.ts" />
/// <reference path="LINQ/StringProvider.ts" />
/// <reference path="Locale/Locale.ts" />
/// <reference path="Locale/LocaleDefinition.ts" />
/// <reference path="Platform/IPlatform.ts" />
/// <reference path="Platform/NodeWebkitPlatform.ts" />
/// <reference path="Resource/Cache.ts" />
/// <reference path="Resource/Loader.ts" />
/// <reference path="Resource/Model/JS.ts" />
/// <reference path="Resource/Model/STL.ts" />
/// <reference path="Resource/Shaders/Shader.ts" />
/// <reference path="Resource/Sound/WAV.ts" />
/// <reference path="Resource/Texture/JPG.ts" />
/// <reference path="Resource/Texture/PNG.ts" />
/// <reference path="Resource/Video/OGV.ts" />
/// <reference path="Scenes/Serializer/AbilityDefinition.ts" />
/// <reference path="Scenes/Serializer/BindingDefinition.ts" />
/// <reference path="Scenes/Serializer/EntityDefinition.ts" />
/// <reference path="Scenes/Serializer/EntityParser.ts" />
/// <reference path="Scenes/Serializer/MaterialDefinition.ts" />
/// <reference path="Scenes/Serializer/ObjectDefinition.ts" />
/// <reference path="Scenes/Serializer/RawShaderDefinition.ts" />
/// <reference path="Scenes/Serializer/SceneDefinition.ts" />
/// <reference path="Scenes/Serializer/SceneSerializer.ts" />
/// <reference path="Scenes/Serializer/ShaderDefinition.ts" />
/// <reference path="Sound/Channel.ts" />
/// <reference path="Sound/Effect.ts" />
/// <reference path="Sound/Mixer.ts" />
//grunt-end