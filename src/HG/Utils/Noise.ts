...
* @Last Modified time: 2013 - 12 - 02 17:22:51
*/
...
module HG.Utils {
	...
	export class Noise {
		...
		static perm: number[] = [151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7,
		...
		static generate3(x: number, y: number, z: number): number {
			...
			if (t0 < 0.0) {
				...
				t0 *= t0;
				n0 = t0 * t0 * Noise.grad3(Noise.perm[ii + Noise.perm[jj +
					Noise.perm[kk]]], x0, y0, z0);
			}
			...
			if (t1 < 0.0) {
				...
				t1 *= t1;
				n1 = t1 * t1 * Noise.grad3(Noise.perm[ii + i1 +
					Noise.perm[jj + j1 + Noise.perm[kk + k1]]], x1, y1, z1);
			}
			...
			if (t2 < 0.0) {
				...
				t2 *= t2;
				n2 = t2 * t2 * Noise.grad3(Noise.perm[ii + i2 + Noise.perm[jj + j2 +
					Noise.perm[kk + k2]]], x2, y2, z2);
			}
			...
			if (t3 < 0.0) {
				...
				t3 *= t3;
				n3 = t3 * t3 * Noise.grad3(Noise.perm[ii + 1 + Noise.perm[jj + 1 +
					Noise.perm[kk + 1]]], x3, y3, z3);
			}
